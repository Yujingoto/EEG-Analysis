...
# 線形代数
線形代数とは**「ベクトルや行列を扱う数学」**であり，線形な関係を記述・操作する理論体系です．与えられた様々な数を，それらの関係を考慮して適切に並べてベクトルや行列の形で表し，その空間で様々な操作をすることによってデータの性質や関係性を明らかにするのが目的です．最大の利点は，一見全く異なるようなデータであっても，一度適切に行列の形に表すことができれば，あとは体系だった同じ操作を適用するだけで対等に扱えることです．

## 脳波解析 (神経科学) と線形代数
線形代数を学ぶ意義は，一言でいえばあらゆるデータは行列だからです．たとえば脳波データは基本的に，チャンネル数と時間の２次元で構成された行列です．ここから，脳を力学系として捉え，その行列を解析して種々の性質を議論する手法が適用されます（[非線形ダイナミクス](../../Physics/NonlinearDynamics/0dynamics.html)など）．また行列であるが故に [PCA](../Others/PCA.html), [ICA](../Others/ICA.html), [CCA](../Others/CCA.html) といった固有値分解・特異値分解による信号分離の手段が適用できます．また周波数分解に用いる[フーリエ変換](../Analysis/fourier.html)や[ウェーブレット変換](../Analysis/wavelet.html)の理解にも線形代数の知識は欠かせません．

あるいはダイポール推定や各種モデルフィッティングで用いる[最小二乗法](../Statistics/linear_regression.html#最小二乗法)なんかも線形代数ですし，空間ラプラシアンフィルタもそうです．行列の代数と言えば，多変量の統計にも当然重要です．ということで，脳波に関わらず数値データ解析に関わるおよそあらゆる操作が線形代数を利用しているのです．これらを理解する上で，線形代数は必須です．

また，より発展的には線形代数の知識を駆使することで [GPU による計算の最適化](../../Simulation/Setup/2gpu.html)なども出来るようになります．
...
行列 $\bm{A}$ が簡単な形になっていたのは，見ればわかりますが対角成分以外が $0$ という形を取っているからです．これによって $x,y$ 軸以外の方向が関係なくなっているわけです．
...
という形で表せます．これを満たす $\bm{x}$ は，$\bm{x} = \bf{0}$ が自明解として存在しますが，これは除きます．なぜなら，点は軸になりえないからです．
...
の**行列式が 0** であることを満たす必要がある，ということになります．
...
たとえば，[主成分分析](../Others/PCA.html)や[力学系の線形安定性解析](../../Physics/NonlinearDynamics/1stability.html)なんかにはダイレクトに使います．
