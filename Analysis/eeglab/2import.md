...
### .eeg 
`.eeg` は [BrainVision Recorder](https://brainvision.com/) などで使われる拡張子です．少なくとも日本だと多くの人はこの拡張子だと思います．ややこしいことに，この形式では一般的に 3 つのファイルをセットで用います．バイナリ形式になった脳波時系列データが格納された `.eeg`, ヘッダー (実験時間や電極などの補足情報) 情報が格納された `.vhdr` (header), そしてイベント (実験時に刺激呈示や回答のタイミングを記録するためのタイムスタンプ) 情報が格納された `.vmkr` (marker) です．`.vhdr`, `.vmkr` を読み込めないと，`.eeg` だけではまともな解釈が出来ません．また， `.vhdr` は同じ名前のファイルに対して働くのでこの 3 つは必ず同じ名前にしておきましょう．

これを読み込むため，[EEGLAB 環境構築](./1setup.md) では `bva-io` プラグインをダウンロードしました．

### .edf
`.edf` は単一ファイルに全てのデータが格納されているため，`.eeg` のように複数のファイルを参照する必要はありません．ただしその分，ヘッダ情報などを簡単に人間が読めるようにはなっていない (はず) です．また，`.edf` はイベント情報を格納する仕組みを持っていないため，別のチャンネル扱いして追加の "Trigger" などを波形として読み込んでおくのが一般的です．脳波解析の際にはこれを閾値処理するなどしてイベント情報を復元します．

### .edf+
`.edf` の拡張版です．この場合，イベント情報 (アノテーション) も組み込むことができます．

## インポート
さて，インポートとはデータの読み込みのことです．EEGLAB にこれから解析する脳波データを読み込ませる作業ですね．簡単です.
ここでは，Brain Vision の形式である `.eeg` ファイルを読み込む方法を紹介します．こいつが一番多いんじゃないですかね？
...
こんな画面が出てきます．どのチャンネルの脳波データを読み込むかとかなので，通常は何もしなくて良いです．書いてある通り，何も記入しなければ全てのチャンネルの全てのデータを読み込んでくれます．なので空欄のまま OK を押します．
