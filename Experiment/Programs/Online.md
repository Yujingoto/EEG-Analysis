...
## 実験本番の流れと注意点
ここまでで基本的な使い方はOKです．本実験として進めるには，Pavlovia の status を `RUUNING` にして配布できる URL を作成し，CrowdWorks などに登録して募集・データ取得，Pavlovia から Results を一括ダウンロードという流れです．ただし， `RUUNING` で実験を走らせるには，1人あたり £0.24 かかります．[参考](https://pavlovia.org/docs/credit-license/overview)．

前項まで，Pavlovia の status は Pilot だったので結果はローカルに保存され，サーバーには空データでした．本番では逆になります．URLも固定です．これらのテストをしたければ課金になります．`RUNNING` および課金については一旦後で．


先に本番の注意点として，**終了画面が出る前にタブを閉じられるとデータが保存されません．**

実際，pilot 段階でも実験の途中で終了してしまうと下記のように'aborted'になります．これはデータの取得が出来てない状態なので，被験者には強く念押ししておく必要があります．
...
### 完了コード
CrowdWorks 側での仕事完了の提出のため，完了コードを日付やparticipant入力値を組み合わせて自動生成し，知らせるなどの工夫が必要になります．
...
<center><img src="../figures/psychopy_pavlovia17.png"></center>

このようにし，`code` の中身は `Code type` を Both とし，左側(phthon)と右側 (JS) にそれぞれ対応するコードを書いていきます．まず `Before experiment` で変数を初期化し，

<center><img src="../figures/psychopy_pavlovia18.png"></center>
...
<center><img src="../figures/pavlovia4.png"></center>

ここで，新しく下に表示される窓には `Assigned`, `Available`, `Reserved`, `Consumed` の枠にそれぞれ数字があります．これが次項で見る Participant credit の内訳です．

**Available** は現在使用可能な（購入済みの）Credit 数です．お金がないので今は 0．

**Assign** は，Available以下なら好きなだけ数字を入れられます．ここにいれて，`Update assined credits` を押すと，その数の credit が **Reserved** に反映されます．Reserved の数だけ，24 時間以内に利用可能（実験可能）になります．使われなかった credit は 24 時間後に Available に戻されます．（つまり 24 時間ごとに Update しないといけないということです．多分）

最後に，24 時間以内に行われた実験数＝消費された credits **がConsumed** に表示されます．

次に，メニューの右上を見ると Running がどういった条件で実行されているかが確認できます．


<center><img src="../figures/pavlovia5.png"></center>

ここには有効なブラウザや機器の一覧があります．`coming soon` とあるので，近いうちにどれに対応させるかを自分で切り替えられるようになるんですかね？今は切り替えられません．

また，実験ページの URL も固定で生成され，表示されています．実験参加者にはこの URL を案内することになります．ただし，今みたいに使える (Reserved) Credit がない状態で誰かがリンクを開いたりすると

<center><img src="../figures/pavlovia6.png"></center>

このようなエラーが実験参加者には表示されます．うっかり更新を忘れていたりするとこうなるわけですね．これ日本語にしておかないと実験参加者が困った時に我々にメールとかできないですね．変えられるのかな．ちょっと分かりません．

### Credit の購入
ということで，ここから先はお金がないと進みません．とはいえ1回程度なら大した額じゃないですが...

Participant credit は [Pavlovia Store](https://pavlovia.org/store/credits) で購入できます．ページのリンクを踏んでログインすると，このような画面にいきます．

<center><img src="../figures/pavlovia3.png"></center>


カードなどの情報を登録し，購入する Credit (1000 credit = £240) を指定しましょう．

購入できたらあとは `RUNNING` にしましょう．`Available` な Credit が増えているはずなので，適切な（欲しいデータ）数だけ Assign し，Update すれば実験が出来るようになります．